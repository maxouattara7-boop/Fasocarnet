...
export const DebtPaymentModal: React.FC<DebtPaymentModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full max-w-sm rounded-t-[32px] sm:rounded-3xl overflow-hidden shadow-2xl p-6 space-y-4.5 animate-in slide-in-from-bottom duration-200 border border-slate-100">
        {!successReceipt ? (
          <>
            <div className="flex items-center justify-between border-b border-slate-100 pb-3.5">
              <div>
                <h3 className="font-black text-slate-900 text-lg tracking-tight font-display">Encaisser un Remboursement</h3>
                <p className="text-xs text-slate-500 font-medium">Client : <span className="font-bold text-slate-800">{customer.name}</span></p>
              </div>
            ...
            <div className="bg-amber-50/80 p-4 rounded-2xl border border-amber-200 flex items-center justify-between">
              <span className="text-[11px] font-black text-amber-800 uppercase tracking-wider font-display">Dette Actuelle :</span>
              <span className="text-lg font-black text-amber-900 tracking-tight font-display">{formatCurrency(customer.totalDebt)}</span>
            </div>

            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-[11px] font-black text-slate-600 uppercase tracking-wider font-display">Montant versé</label>
                <button
                  type="button"
              ...
              <input
                ...
                className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-lg text-slate-900 focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all"
              />
            </div>

            {/* Mode de paiement */}
            <div>
              <label className="block text-[11px] font-black text-slate-600 uppercase tracking-wider mb-2 font-display">Mode d'encaissement</label>
              <div className="grid grid-cols-4 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`py-2.5 text-xs font-bold rounded-2xl border-2 transition-all font-display ${
            ...
            <div className="pt-2 flex space-x-2.5">
              ...
              <button
                ...
                onClick={handlePay}
                className="w-2/3 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl text-xs shadow-lg shadow-emerald-600/30 active:scale-98 transition-all flex items-center justify-center space-x-1.5"
              >
          ...
          <div className="text-center space-y-4 py-3">
            ...
            <div>
              <h3 className="text-xl font-black text-slate-900 tracking-tight font-display">Règlement Enregistré !</h3>
              <p className="text-xs text-slate-500 mt-1">
                Versement de <span className="font-black text-emerald-700">{formatCurrency(successReceipt.amountPaid)}</span>
              </p>
            ...
            <div className="space-y-2 pt-2">
              <button
                ...
                className="w-full py-3.5 bg-[#25D366] hover:bg-[#20bd5a] text-white font-black rounded-2xl text-xs shadow-lg shadow-green-600/25 flex items-center justify-center space-x-2 active:scale-98 transition-all"
              >
